...
for(draw in 1:ndraws) {
  ...
  if ( CD3s[row+draw, 4] == 1 ) { # if there is an edge from treatment to CD3, impute
  ...
  if ( CD8s[row+draw, 4] == 1 ) { # if there is an edge from treatment to CD8, impute
    ...
    cd8draw1 = rnorm(n, mean = cd81, sd = sqrt( CD3s[row + draw, 8]) ) # fixed treatment to 1
    cd3draw1 = ifelse(dat$A2_0 == 1, log(dat$CD3N_1), cd3draw1)
  ...
  if ( VLs[row+draw, 6] == 1 ) { # if there is an edge from treatment to VL, impute
    ...
    VL0 = cbind(1, dat$VL_1, 1) %*% unlist(flatten( VLs[row+draw, 1:3]))  # eta
    VLdraw0 = rbinom(n, 1, prob = plogis(VL0)) # fixed treatment to 1
    VLdraw0 = ifelse(dat$A2_0 == 1, VL0, VLdraw0)
  } else {
  ...
  my0 = cbind(1, cd4draw1, cd8draw1, cd3draw1, VLdraw1, 0, 0) %*% unlist(flatten(Ys[(row+draw), 9:15])) # always treated
  Ydraw0 = rnorm(n, mean = my1, sd = (Ys[(row+draw), 16])^(1/4) )
...
pp$y11 = Ydraw1
pp$y00 = Ydraw0
pp2 = filter(pp, t1==1) %>% select(CASEID, CD4N_2, y11) 
