...
labsdat.gf = labsdat.gf %>% group_by(CASEID) %>% mutate(A2 = c(A[2:4],'na'))
...
histories = c(lagged)
histvars = list(c('CD4N', 'A2'))
covparams = list(covmodels=c(CD4N ~ lag1_CD4N + lag1_A2,
                             A2 ~ CD4N + lag1_A2))
ymodel = Y ~ A2 + lag1_A2 + lag1_CD4N
intvars = list('A2', 'A2')
interventions = list( list(c(static, rep(0,4))),
                      list(c(static, c(1,1,1,1))) )
int_descript = c('Never', 'Always')

b = gformula_continuous_eof(labsdat.gf, id=id, time_name = time_name, covnames = covnames, covtypes = covtypes,
